...
class OllamaReasoningProvider(RuleBasedProvider):
    ...
    def plan_reasoning(self, task_spec: TaskSpec, recipe, mismatches) -> dict[str, object] | None:
        self.call_count += 1
        payload = self._generate_json(build_plan_prompt(task_spec, recipe, mismatches))
        if not payload:
    ...
    def propose_actions(self, task_spec: TaskSpec, recipe, mismatches, plan=None, limit: int = 3) -> list[dict[str, object]] | None:
        self.call_count += 1
        payload = self._generate_json(build_action_proposal_prompt(task_spec, recipe, mismatches, plan=plan, limit=limit))
    ...
    def critique_recipe(self, task_spec: TaskSpec, recipe, verification) -> dict[str, object] | None:
        self.call_count += 1
        payload = self._generate_json(build_critique_prompt(task_spec, recipe, verification))
        if not payload:
